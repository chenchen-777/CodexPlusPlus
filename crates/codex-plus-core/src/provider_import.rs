...
mod tests {
    ...
    fn parses_codexplusplus_provider_url() {
        let url = "codexplusplus://v1/import/provider?resource=provider&name=Example%20Provider&baseUrl=https%3A%2F%2Fexample.test%2Fv1&apiKey=sk-test&wireApi=responses&relayMode=pureApi&configContents=bW9kZWxfcHJvdmlkZXIgPSAiQ29kZXhQbHVzUGx1cyIK&authContents=eyJPUEVOQUlfQVBJX0tFWSI6InNrLXRlc3QifQo%3D";

        let request = request_from_url(url).unwrap();

        assert_eq!(request.name, "Example Provider");
        assert_eq!(request.base_url, "https://example.test/v1");
    ...
    fn pending_provider_import_round_trips_and_clears() {
        ...
        assert_eq!(pending.base_url, "https://example.test/v1");
